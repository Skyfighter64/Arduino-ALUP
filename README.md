...
To set the number of LEDs, simply count the LEDs on your adressable LED strip. If you want to just use part of this strip, simply decrease it. This value has to be  at least 1 and can be a maximum of 715 827 882 (limited by the maximum frame body size), even though the practical maximum number is lower depending on the amount of RAM of the microcontroller. 
...
The baud rate represents the amount of bits per second transfered over the serial connection. The maximum value depends on the used hardware used. When it is too high, you may experience `Frame Errors`, but its default value should cause no problems.
...
The `DEVICE_NAME` can be set to aything you'd like. Its use is to distinguish between different devices if multiple microcontrollers using this sketch are connected to the same computer, so try to use differen names for each device you configure. 
